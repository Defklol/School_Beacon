...
#define   LINE_VALUE_CHAR_UUID            0x1409
...
/**@brief   Beacon Service @ref BLE_BCN_EVT_LORA_INTERVAL event data.
 *
 * @details This structure is passed to an event when @ref BLE_BCN_EVT_LORA_INTERVAL occurs.
 */
typedef struct
{
    uint8_t const * p_data;
...
/**@brief   Beacon Service @ref BLE_BCN_EVT_LINE event data.
 *
 * @details This structure is passed to an event when @ref BLE_BCN_EVT_LINE occurs.
 */
typedef struct
{
    uint8_t const * p_data;
    uint16_t        length;
}ble_bcn_evt_line_data_t;
...
typedef struct
{
    ...
    union
    {
        ...
        ble_bcn_evt_line_data_t             line_data;
    }params;
}ble_bcn_evt_t;
...
struct ble_bcn_s
{
    ...
    ble_gatts_char_handles_t      line_value_handles;             /**< Handles related to the Line Value characteristic. */
...
static uint32_t line_change_value_char_add(ble_bcn_t * p_cus, const ble_bcn_init_t * p_cus_init);
