...
#define BLE_BCN_MAX_MINOR_CHAR_LEN            5        /**< Maximum length of the RX Characteristic (in bytes). */
...
#define BLE_BCN_MAX_LINE_CHAR_LEN             5
...
static void on_write(ble_bcn_t * p_bcn, ble_evt_t const * p_ble_evt)
{
    ...
    {
        evt.params.txpower_data.p_data          = p_evt_write->data;
        evt.params.txpower_data.length     = p_evt_write->len;
    ...
    {
        evt.params.address_data.p_data     = p_evt_write->data;
        evt.params.address_data.length     = p_evt_write->len;
    ...
    {
        evt.params.ad_interval_data.p_data     = p_evt_write->data;
        evt.params.ad_interval_data.length     = p_evt_write->len;
    ...
    {
        evt.params.lora_interval_data.p_data     = p_evt_write->data;
        evt.params.lora_interval_data.length     = p_evt_write->len;
        evt.evt_type                     = BLE_BCN_EVT_LORA_INTERVAL_DATA;
        p_bcn->evt_handler(p_bcn, &evt);
      
    }
    */
    if(p_evt_write->handle == p_bcn->line_value_handles.value_handle)
    {
        evt.params.line_data.p_data     = p_evt_write->data;
        evt.params.line_data.length     = p_evt_write->len;
        evt.evt_type                    = BLE_BCN_EVT_LINE_DATA;
        p_bcn->evt_handler(p_bcn, &evt);
    }
}
...
uint32_t ble_bcn_init(ble_bcn_t* p_bcn, const ble_bcn_init_t* p_bcn_init)
{
  ...
  */
  /*
  err_code = line_change_value_char_add(p_bcn, p_bcn_init);
  if(err_code != NRF_SUCCESS)
  {
      return err_code;
  }
...
/**@brief Function for adding the Line Value characteristic.
 *
 * @param[in]   p_bcn        Beacon Change Service structure.
 * @param[in]   p_bcn_init   Information needed to initialize the service.
 *
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t line_change_value_char_add(ble_bcn_t * p_bcn, const ble_bcn_init_t * p_bcn_init)
{
  ble_gatts_char_md_t       char_md;
  //ble_gatts_attr_md_t       cccd_md;
  ble_gatts_attr_t          attr_char_value;
  ...
  ble_uuid.type             = p_bcn->uuid_type;
  ble_uuid.uuid             = LINE_VALUE_CHAR_UUID;
  ...
  attr_char_value.init_len  = sizeof(uint8_t);
  attr_char_value.init_offs = 0;
  attr_char_value.max_len   = BLE_BCN_MAX_LINE_CHAR_LEN;
  
  return sd_ble_gatts_characteristic_add(p_bcn->service_handle,
                                         &char_md,
                                         &attr_char_value,
                                         &p_bcn->line_value_handles);  
}
